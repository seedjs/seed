...
var Remote = Co.extend({
  ...
  */
  fetch: function(packageInfo, done) {
    ...
    this.download(filepath, url, function(err){
      ...
      Co.exec('unzip ' +filepath+' -d '+path, function(err) {
        if (err) return done(err);
  ...
  */
  login: function(usernames, done) {   
    ...
    // collect username
    Co.chain(function(done) {
      Co.println("Login to "+remote.url);
    ...
    // go get a token from remote...
      ...
      remote.request('POST', 'tokens', opts, function(err, status, data) {
        ...
        var tokenId = data.id;       
        if (!tokenId) {
          return done("Server did not return token (data="+Co.inspect(data)+")");
        }
  ...
  */
  logout: function(usernames, done) {
    ...
    if (!usernames) {
      Co.println("Removing all tokens for "+remote.url);
    } else {
      Co.println("Removing tokens for "+usernames.join(',')+" on "+remote.url);
    }
  ...
  */
  signup: function(done) {
    ...
    // get username
    Co.chain(function(done) {
      Co.println("Create new account for " + remote.url);
      Co.println("Username:");
      ...
      Co.fs.glob(pkg.path, function(err, paths) {
        ...
        Co.each(paths, function(srcname, done) {
          ...
          Co.fs.mkdir_p(Co.path.dirname(dst), 511, function(err) {
            if (err) Co.println('mkdir_p ' + Co.path.dirname(dst));
            if (err) return done(err);
