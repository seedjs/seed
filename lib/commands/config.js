...
function showAllConfigs(sources, done) {
  ...
  function iter(hash, base) {  
    for(var key in hash) {
      ...
      else if (val !== undefined) configs[key] = Co.sys.inspect(val);
    }
  }
...
exports.invoke = function(cmd, args, opts, done) {
  ...
  // read config
    ...
    if (key.indexOf('.')>=0) {
      ...
      Co.sys.puts(key + ': '+Co.sys.inspect(ret));
    }
    done();

  // replace config
  } else {
    Co.sys.debug(Co.sys.inspect(sources.map(function(src) { return src.config.path; })));
    ...
    if (key.indexOf('.')>=0) {
      ...
      Co.sys.puts(key + ': '+Co.sys.inspect(src.info(key)));
      Co.sys.debug(src.config.path);
