...
exports.invoke = function(cmd, args, opts, done) {
  ...
  if (!exports[cmd]) {
    Co.println('Unknown command "remote '+cmd+'"\n');
...
function remoteUrlRequired(done) {
  Co.println("Remote URL required");
  done();
}
...
exports.show = function(cmd, args, state, done) {
  REMOTE.config(function(err, remotes) {
    ...
    if (remotes.length===0) {
      Co.println('(No remotes)');
    } else {
      remotes.forEach(function(remote) {
        Co.println(remote.url + ' ('+remote.moduleId+')');
      });
    }
    done();
  });
...
exports.add = function(cmd, args, state, done) {
  ...
  REMOTE.config.add(state.domain, remoteUrl, remoteType, function(err,flag){
    if (err) return done(err);
    if (flag) Co.println('Added remote '+remoteUrl+' ('+remoteType+')');
    else Co.println('Remote '+remoteUrl+' already exists');
    return done();
  });
};
...
exports.remove = function(cmd, args, state, done) {
  ...
  REMOTE.config.remote(state.domain, remoteUrl, function(err, flag) {
    if (err) return done(err);
    if (flag) Co.println("Removed remote "+remoteUrl);
    else Co.println(remoteUrl+" is not a remote in domain "+state.domain);
    return done();
  });
};
...
exports.list = function(cmd, args, state, done) {
  ...
  function(info, packages, done) {
    ...
    names.forEach(function(name) {
      if (state.details) {
        Co.println(LIST_CMD.detailsFor(combinedPackages[name]));
      } else {
        Co.println(LIST_CMD.summaryFor(name, info[name]));
      }
    });
    return done();
  })(done);
...
exports.login = function(cmg, args, state, done) {
  ...
  remoteFor(state.remote, function(err, remote) {
    ...
    return remote.login(usernames, Co.err(done));
  });
};
...
exports.logout = function(cmg, args, state, done) {
  ...
  remoteFor(state.remote, function(err, remote) {
    if (err || !remote) return done(err || '(No Remote)');
    return remote.logout(usernames, Co.err(done));
  });
};
...
exports.signup = function(cmd, args, state, done) {
  remoteFor(state.remote, function(err, remote) {
    if (err || !remote) return done(err || '(No Remote)');
    return remote.signup(Co.err(done));
  });
};
