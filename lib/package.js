...
/**
  Returns ALL modules defines known to the current package.  Optionally 
  name the kine of modules you want to load.  Not passing a kind will get
  the package's default modules only.  Usually this is what you want.
*/
Package.prototype.catalogModules = function(kind) {
  ...
  var base = this.path,
      ...
      moduleIds  = [],
      seen       = {},
      extensions = Object.keys(this.getReaders());
  ...
  dirs.forEach(function(dir) {
    dir = CORE.path.join(base, dir);
    CORE.fs.glob(dir).forEach(function(pathname) {
      
      var moduleId, ext;
      
      // only consider moduleId's we know
      ext = CORE.path.extname(pathname);
      if (extensions.indexOf(ext)<0) return;
      
      // moduleId is pathname excluding extension.  skip if seen
      moduleId = pathname.slice(0, 0 - ext.length);
