...
function sliceDummyext(path) {
  ...
  if (path.slice(loc) === ext) {
    return path.slice(0, loc);
  } else return path;
}
...
var AnonymousPackage = Package.extend({
  ...
  findModulePath: function(moduleId, extensions, done) {
    ...
    if (moduleId[0] === '/') {
      var path = sliceDummyext(moduleId);
      Co.path.exists(path, function(err, exists) {
        if (err) return done(err);
        if (!exists) return done(null, null); // not found
        else return done(null, moduleId); // leave dummyext for type detect
      });
