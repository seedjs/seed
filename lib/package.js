  ...
  normalize: function(key, value) {
    switch(key) {
      case 'lib':
        ...
        break;
      case 'test':
        if (!value) value = ['test', 'tests', 'spec'];
        if ('string' === typeof value) value = [value];
        break;
    }
    return value;
...
Package = Co.extend(Resource, {
  ...
  */
  loadModule: function(sandbox, moduleId, done) {
    ...
    // also
    function findModulePath(moduleId, done) {
      pkg.findModulePath(moduleId, function(err, path) {
        if (!err && !path) {
          if (moduleId === 'index') return findModulePath(pkg.name(), done);
          else err = new Error(pkg.name() + ':' + moduleId + ' not found');
        }
        return done(err, path);
      });
    }
    
    findModulePath(moduleId, function(err, path) {
      if (err) return done(err);
