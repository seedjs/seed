...
Seed = Co.extend({
  ...
  */
  resolve: function resolve(moduleId, curModuleId, curPackage) {
    ...
    // if id does not contain a packageId and it starts with a / then always 
    // use the default package to provide compatibility with node.js require
    if (moduleId[0]==='/' && moduleId.indexOf(':')<0) packageId = 'default'; 
  ...
  */
  defaultModulePath: function(moduleId, callback) {
    ...
    var paths = process.paths.map(function(path) {
