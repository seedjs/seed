...
Seed = Co.extend({
  ...
  NATIVE_MODULES: function() {
    ...
    // node 0.1.33
    if (process.binding) {
      ret = Object.keys(process.binding('natives'));
  ...
  debug: function() {
    ...
    Co.debug(Array.prototype.slice.call(arguments).join(''));
    return this;  
  },
  ...
  */
  register: function(packagePath, callback) {
    ...
    if ('string' === typeof packagePath) {
      ...
      if ('string' === typeof ret[ret.length-1]) callback = null;
      else callback = ret.pop();

      packagePath = Co.path.normalize(Co.path.join.apply(Co.path, ret));
  ...
  */
  canonical: function canonical(moduleId, curModuleId, curPackage, done) {
    ...
    if (cache && cache[resolvedId]) {
      this.debug("  Found in cache: "+cache[resolvedId]);
    ...
    if (resolvedId.indexOf(':')<0) {
      ...
      // search in curPackage first for matching module name
      (function(next) {
        
        // handle anonymous packages
        if (resolvedId[0]==='/') {
          return action(joinPackageId('(anonymous)', resolvedId));
        }
        
        if (curPackage) {
          loader.debug('searching curPackage: ', curPackage.name());
          
          curPackage.moduleExists(resolvedId, function(err, exists) {
            if (exists) {
              loader.debug("FOUND: in current package");
              action(joinPackageId(curPackage.name(), resolvedId));
            } else return next();
          });
          
        } else return next();

      // not in current package or there is no current package.  look for 
      // default module
      })(function() {
        loader.debug('searching for default module: ', resolvedId);
        loader.defaultModuleExists(resolvedId, function(err, exists) {   
          if (exists) {
  ...
  */
  _canonical: function(resolvedId, qualifiedId, curPackage, done) {
    ...
    // shortcut for anonymous package
    if (packageId === '(anonymous)') {
      ret = '::' + qualifiedId;
      if (curPackage) {
        if (!curPackage.canonicalCache) curPackage.canonicalCache = {};
        curPackage.canonicalCache[resolvedId] = ret;
      }
      if (!packagesByCanon['::(anonymous)']) {
        packagesByCanon['::(anonymous)'] = Package.anonymous();
      }
      this.debug('returning anonymous '+ret);
      return done(null, ret);
    }
    ...
    if (curPackage && (curPackage.name() === packageId)) {
      ...
      if (!curPackage.canonicalCache) curPackage.canonicalCache = {};
      curPackage.canonicalCache[resolvedId] = ret ;
      packagesByCanon['::'+curPackage.canonicalId()] = curPackage;
      this.debug('returning curPackage '+resolvedId+'='+curPackage.path);
      return done(null, ret);
    }
    ...
    function(vers, foundPackage, done) {      
      ...
      var sources = seed.sources;
      seed.debug(sources.map(function(s) { return s.path; }));
      
      Co.find(sources, function(source, done) {
        if (!source) return done(null, false);

        seed.debug("  searching in "+source.path);
  ...
  */
  ...
  /**
    Retrieves a copy of the named info from all known packages.
    Useful for detecting plugin configs named in packages.
  */
  collectPluginInfo: function(workingPackage, key, done) {
    var results = this._pluginInfo, seed = this;
    if (!results) results = this._pluginInfo = {};
    if (!results[key]) {
      results[key] = Co.once(function(done) {
        seed.preferredPackageList(workingPackage, function(err, list) {
          if (err) return done(err);
          Co.reduce(Object.keys(list), {}, function(ret, packageId, done) {
            seed.openPackage(packageId, list[packageId][0], workingPackage, function(err, pkg) {
              if (err || !pkg) return done(err || (packageId+' not found'));
              var config = pkg.info(key); // look for a seed config
  ...
  /**
    Returns a merged copy of all plugin info for the named key
  */
  collectMergedPluginInfo: function(workingPackage, key, done) {
    var seed = this;
    seed.collectPluginInfo(workingPackage, key, function(err, info) {
      var ret = {};
      Object.keys(info).forEach(function(packageId) {
        Object.keys(info[packageId]).forEach(function(key) {
          var v = info[packageId][key];
          if (v.indexOf(':')<0) v = packageId+':'+v; 
          ret[key] = v;
        });
      });
      return done(null, ret);
  ...
  */
  openPackage: function(packageId, vers, curPackage, done) {
    // if the package is a path, then just open from disk.
    if (packageId[0]==='/') {
      done = vers; // others are ignored
      return Package.open(packageId, null, done);
    } else {
      return this._openPackage(packageId, vers, curPackage, true, done);
    }
  },
  
  /**
    Opens a compatible package with the given packageId.  If you pass a vers
    opens the specific version or a compatible version - otherwise uses the
    version for the current package.
  */
  openCompatiblePackage: function(packageId, vers, curPackage, done) {
    return this._openPackage(packageId, vers, curPackage, false, done);
  },
  
  _openPackage: function(packageId, vers, curPackage, exact, done) {
    ...
    Co.find(sources, function(source, done) {
      source.compatiblePackage(packageId, vers, function(err, pkg) {
        // must be exact
        if (exact && pkg && vers && (pkg.version() !== vers)) pkg = null;
        ret = pkg;
        done(null, !!pkg);
      });
    })(function(err) { 
      if (err) return done(err);
      if (!ret) return done();
      else return ret.open(done);
   });
    
  },
  
  /**
    Returns a package matching the first path that appears to be an actual
    package or null if none could be found.  A directory is assumed to be a
    package if it has a package.json file.
  */
  openNearestPackage: function(path, done) {
    
    function detectPackage(path, done) {
      if (path === '.') return done(null, null); // not found
      
      Package.detect(path, function(err, isPackage) {
        if (err) return done(err);
        if (!isPackage) detectPackage(Co.path.dirname(path), done);
        Package.open(path, null, function(err, pkg) {
          if (err) return done(err);
          if (!pkg) detectPackage(Co.path.dirname(path), done);
          return done(null, pkg);
        });
      });
    }
    
    detectPackage(path, done);    
  },
  ...
  */
  defaultModulePath: function(moduleId, callback) {
    ...
    paths = require.paths || process.paths || [];
