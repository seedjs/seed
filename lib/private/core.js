...
// check that platform version if OK if provided
if (platform.REQUIRED_VERSION) {
  if (core.semver.compare(platform.VERSION, platform.REQUIRED_VERSION)<0) {
    throw new Error("Requires at least platform version "+
        platform.REQUIRED_VERSION+" (current version: "+platform.VERSION+")");
  }
}
