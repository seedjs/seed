...
/**
  ...
  Private continuables-based library used throughout Seed.  Inspired by the Do
  library.  Do not use this library outside of seed.  If you want this same
  functionality, use the 'co' package instead.
*/
...
function _Cextend() {
  ...
  args.unshift(this);
  return Co.extend.apply(Co, args);
}
...
Co.extend = function(F) {
  ...
  if ('function' === typeof F) {
    ...
    if (F.extend) Ret.extend = F.extend;
  ...
  if (!Ret.extend) Ret.extend = _Cextend;
...
Co.without = function(array, value) {
  var loc, found, lim, ret, k;
  
  // optimize case where value is not found in array
  var values = Array.prototype.slice.call(arguments, 1);
  loc = values.length;
  found = false;
  while(!found && (--loc>=0)) found = array.indexOf(values[loc])>=0;
  ...
  for(loc=0;loc<lim;loc++) {
    k = array[loc];
    if (values.indexOf(k)<0) ret[ret.length]=k;
  }
  return ret;
};
...
/** 
  ...
  @returns {Array}
*/
Co.uniq = function(array) {
  var ret = [], lim = array.length, idx, k;
