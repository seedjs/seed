...
if (!mod) {
  mod = core.nativeRequire('sys');
  platform.exec = function(cmd, echo, callback) {
    if (echo) {
      core.println('WARN: echoing child process output not supported on this platform');
    }
    return mod.exec(cmd, callback);
  };
}
